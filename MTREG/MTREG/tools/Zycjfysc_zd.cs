...
namespace MTREG.tools
{
    class Zycjfysc_zd
    {
        public RetMsg ybscfymx(int mtzyjliid, string grbh, string zyh)
        {
            ...
            string sql_xgbzwl = "update ihsp_costdet set ypspbz = 0 where insursync='N' and ypspbz is null and  ihsp_id = " + mtzyjliid;
        ...
        //先负担后金额甲乙丙(批量)
        public bool xfdhjejyb_pl(string grbh, int mtzyjliid, string zyh, out string mesg)
        {
            mesg = "";
            JKDB jkdb = new JKDB();
            YBCJ yw1 = new YBCJ();
