...
namespace MTREG.medinsur.hdyb.dor
{
    class Zycjfysc
    {
        ...
        public RetMsg ybscfymx(int ihsp_id, string patienttypeid,string grbh, string zyh, Label txt)
        {
            ...
            while (flag > 0)
            {
                flag = costTransferSub(ihsp_id, patienttypeid,grbh,zyh, out mesg, counts, txt);
                sss = sss + 50;
        ...
        public int costTransferSub(int ihsp_id, string patienttypeid, string grbh,string zyh ,out string mesg, string counts, Label txt)
        {
            mesg = "";
            ...
            for (int i = 0; i < datatable.Rows.Count; i++)
            {
                ...
                string dw = datatable.Rows[i]["unit"].ToString();
                ...
                guige = System.Text.RegularExpressions.Regex.Replace(itemname, "[|<>&]", "");
                dw = System.Text.RegularExpressions.Regex.Replace(itemname, "[|<>&]", "");
                if (dw.Length > 6)
                {
                    dw = dw.Substring(0, 6);
                }
        ...
        {
            mesg = "";
            string his_cx = "select id from ihsp_costdet where ihsp_id='" + mtzyjliid + "' and insursync='Y' and ybxfdhjebz=0;";
            ...
            string sql_gxsfck = "select sfck,status,nhflag,insurcode from inhospital where id='" + mtzyjliid + "'";
            DataSet ds_sfck = BllMain.Db.Select(sql_gxsfck);
            ...
            for (int i = 0; i < dqkcxfdhje_pl_ccs.Length - 1; i++)
            {
                ...
                sql_update_mtzyjlstuff = "update ihsp_costdet set ybxfdhjebz=1,ybxfdhje='" + dqkcxfdhje_pl_cc[3].Trim() + "', yblx = '" + dqkcxfdhje_pl_cc[4].Trim() + "', insurclass = '" + dqkcxfdhje_pl_cc[4].Trim() + "' where ihsp_id=" + mtzyjliid + " and id=" + dqkcxfdhje_pl_cc[0].Trim() + ";";
        ...
        public bool xfdhje(int ihsp_id, string zyh, out string mesg)
        {
            ...
            string ylzh = "";
            mesg = "";
            string his_cx = "select id from ihsp_costdet where ihsp_id='" + ihsp_id + "' and insursync=1 and ybxfdhjebz=0;";
            DataTable dt_his_cx = BllMain.Db.Select(his_cx).Tables[0];
            if (dt_his_cx.Rows.Count == 0)
            {
                return true;
            ...
            {
                ylzh = ds_sfck.Rows[0]["ybgrbh"].ToString().Trim();
            }
            ...
            for (int i = 0; i < dt_kc22_cx.Rows.Count; i++)
            {
                ...
                if (opt_ybfdhje == 0)
                {
                    ...
                    string update_sql = "update ihsp_costdet set ybxfdhjebz=1,ybxfdhje='" + fdhje_cc[0] + "', yblx = '" + fdhje_cc[1] + "' where id=" + dt_his_cx.Rows[i]["id"].ToString().Trim() + ";";
