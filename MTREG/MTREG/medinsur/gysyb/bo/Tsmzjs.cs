...
namespace MTREG.medinsur.gysyb.bo
{
    ...
    class Tsmzjs
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string Tsmzjs_in(string[] parm)
        {
            ...
            data += "<IPADDR>" + parm[4] + "</IPADDR>";//终端机IP地址              使用IC卡及身份证时，通过GETPSNINFO获得的个人编码。
            data += "<PSAMNO>" + parm[5] + "</PSAMNO>";//PASM卡号                  客户端IP地址
            data += "<PASSWORD>" + parm[6] + "</PASSWORD>";//密码
            data += "<INSURETYPE>" + parm[7] + "</INSURETYPE>";//保险类别
            data += "<SPECILLNESSCODE>" + parm[8] + "</SPECILLNESSCODE>";//特种病编码   见特种病编码表
            data += "<STARTDATE>" + parm[9] + "</STARTDATE>";//待遇开始享受时间
            data += "<ISCAL>" + parm[10] + "</ISCAL>";//是否结算                        1：实际结算；0：模拟结算
            ...
            data += "<CFNO>" + parm[14] + "</CFNO>";//处方本编号
            data += "<OPERATOR>" + parm[15] + "</OPERATOR>";//操作员
            data += "<DODATE>" + parm[16] + "</DODATE>";//办理日期

            return data;
        }
        public string Tsmzjs_in2(string[] parm2)
        {
            string data = "<ROWSET>";
            data += "<ROW ITEMCODE=" + "\"" + parm2[0] + "\"";//医保编码
