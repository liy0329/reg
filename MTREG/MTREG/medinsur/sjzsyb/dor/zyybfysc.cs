...
namespace MTREG.medinsur.sjzsyb.dor
{
    class Sjzzyybfysc
    {
        ...
        private int scfymx(int ihsp_id, string counts, string zyh, out string mesg, Label txt)
        {
            ...
            string sql1 = "SELECT "
                            ...
                            + " (SELECT AKC273 FROM contrast_doc WHERE contrast_doc.bas_doctor_id =ihsp_costdet.doctor_id GROUP BY BKF050) AS AKC008, "//中心医师名称
                            ...
                            + " (SELECT AKF002 FROM contrast_dep WHERE contrast_dep.bas_depart_id = ihsp_costdet.ihspdep_id GROUP BY AKF001) AS AKC025,"//中心科室名称
            ...
            foreach (DataRow dr in datatable.Rows)
            {
                ...
                kc22.AKC008 = dr["AKC008"].ToString().Trim();
                kc22.BKF040 = dr["BKF040"].ToString().Trim();
                kc22.AKC025 = dr["AKC025"].ToString().Trim();
        ...
        public int scfymx_at(int ihsp_id, string counts, string zyh, out string mesg)
        {
            ...
            foreach (DataRow dr in datatable.Rows)
            {
                ...
                atsc.CKAA11 = Double.Parse( dr["AKC226"].ToString().Trim()).ToString("0");
            ...
            string Ylzh = "";//医疗证号
            string sql_ybjl = "SELECT AKC190,AKA130 FROM Sybzyjl  WHERE AKC190 = " + DataTool.addFieldBraces(zyh);
            DataTable dtybjl = BllMain.Db.Select(sql_ybjl).Tables[0];
            string yllb = dtybjl.Rows[0]["AKA130"].ToString().Trim();
            
            string sql_gxsfck = "select healthcard,insurcode from inhospital where id='" + ihsp_id + "'";
            DataTable ds_sfck = BllMain.Db.Select(sql_gxsfck).Tables[0];
            string grbh = ds_sfck.Rows[0]["healthcard"].ToString().Trim();
            string iccode = ds_sfck.Rows[0]["insurcode"].ToString().Trim();
            ...
            foreach (DataRow dr in datatable.Rows)
            {
                string iid = "";
                iid = dr["AKC220"].ToString().Trim();
                string sql_ybsc = "UPDATE ihsp_costdet SET inset = 'Y' WHERE id = " + iid.ToString() + ";";
                int z = BllMain.Db.Update(sql_ybsc);
            }
