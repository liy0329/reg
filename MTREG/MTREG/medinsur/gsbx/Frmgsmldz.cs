...
namespace MTREG.gsbx
{
    public partial class Frmgsmldz : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string sql_cx = "select * from mtprod where iid=" + this.iid;
            DataTable dt_cx = BllMain.Db.Select(sql_cx).Tables[0];
            if (dt_cx.Rows.Count == 0)
            {
                sql_cx = "select * from mtjcxm where iid=" + this.iid;
                DataTable dt_cx1 = BllMain.Db.Select(sql_cx).Tables[0];
                if (dt_cx1.Rows.Count == 0)
                    return;
                string sql_gx_mtjcxm = "update mtjcxm set gsbm='" + this.tbxbm.Text.Trim() + "' where iid=" + this.iid;
            ...
            {
                string sql_gx = "update mtprod set gsbm='" + this.tbxbm.Text.Trim() + "' where iid=" + this.iid;
