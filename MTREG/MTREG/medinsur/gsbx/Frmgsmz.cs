...
namespace MTREG.gsbx
{
    public partial class Frmgsmz : Form
    {
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            string startDate = dateTime_cxrqks.Value.ToString("yyyy-MM-dd") + " 00:00:00";
            string endDate = dateTime_cxrqjs.Value.ToString("yyyy-MM-dd") + " 23:59:59";
            ...
            DataTable dt_brxx = BllMain.Db.Select(sql).Tables[0];
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (_iid == "0")
            {
                string sqlzyh = " select nextval('gsjlh');  ";
        ...
        private bool dataUpload()
        {
            ...
            string sql2 = "update mtmzblstuff set clinictype=501 ,gsjlh="+_iid+",rdsh='" + tbx_rdsh.Text.Trim() + "',sfxgs='" + sfxgs + "',ybgrbh=  '" + RydjGrbh.Text.Trim() + "' where iid in( " + iid4 + ");";//修改his系统stuff标志，置为501
            string sql_in = "INSERT INTO gsryinfo(iid, grbh, name, dwmc, scdj, shbw, yllb,jssj)VALUES (" + _iid + ",'" + RydjGrbh.Text.Trim() + "','" + tbx_dkxm.Text.Trim() + "','" + RydjDwbh.Text.Trim() + "','" + tbx_scdj.Text.Trim() + "','" + tbx_shbw.Text.Trim() + "','" + comboYllb.Text.Trim() + "','" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "');";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string sqlTmp = "select gsjlh,clinictype,iid,rdsh  from mtmzblstuff where iid in (  " + iid4+")";
            DataTable dt = BllMain.Db.Select(sqlTmp).Tables[0];
            if (dt.Rows[0]["clinictype"].ToString().Trim() != "501")
            {
            ...
            string sql_up = "update mtmzblstuff set clinictype=136,rdsh='',spbh='' where iid in(" + iid4+") ; ";
            sql_up += " delete from gsryinfo where iid=" + _iid+" ;";
            if (BllMain.Db.Update(sql_up) == -1)
            {
                SysWriteLogs SysWriteLogs = new common.bll.SysWriteLogs();
                SysWriteLogs.writeLogs("工伤回退更新his错误信息", DateTime.Now, "sql=" + sql_up);
        ...
        private void btnXfsj_Click(object sender, EventArgs e)
        {
            ...
            string sql_sfjs = "select clinictype from mtmzblstuff where iid  in( " + iid4+")";
            DataTable dtxx_sfjs = BllMain.Db.Select(sql_sfjs).Tables[0];
            if (dtxx_sfjs.Rows[0]["clinictype"].ToString() != "501")
                return;
            ...
            string sql_mzbr = " select mtstuffitem.iid as cfh,mtstuffitem.fph as fph,Substring(mtstuffitem.createdat,0,11) as cfsj,'1' as cfts,mtprod.gszf,mtprod.gsbm as xmbm,mtprod.name as xmmc, gsxmlb.id as sflb,case  when (mtstuffitem.projecttype='2') or (mtstuffitem.projecttype='3' ) or (mtstuffitem.projecttype='4') then '0' else '1' end as zl,mtstuffitem.prc,mtstuffitem.qty as num,mtstuffitem.amt,mtprod.prodguige as guige,prodjixing.name as jixing ";
            sql_mzbr += " from mtstuffitem,mtprod,prodjixing,gsxmlb where   mtstuffitem.mtprod=mtprod.iid and gsxmlb.hisid=mtstuffitem.projecttype and prodjixing.iid=mtprod.prodjixing and mtstuffitem.ybsfsc=0 and mtmzblstuff in( " + iid4 + " )";
            sql_mzbr += " union ";
            sql_mzbr += " select mtstuffitem.iid as cfh,mtstuffitem.fph as fph,Substring(mtstuffitem.createdat,0,11) as cfsj,'1' as cfts,mtjcxm.gszf,mtjcxm.gsbm as xmbm,mtjcxm.name as xmmc, gsxmlb.id as sflb,case  when (mtstuffitem.projecttype='2') or (mtstuffitem.projecttype='3' ) or (mtstuffitem.projecttype='4') then '0' else '1' end as zl,mtstuffitem.prc,mtstuffitem.qty as num,mtstuffitem.amt,'' as guige,'' as jixing ";
            sql_mzbr += " from mtstuffitem,mtjcxm,gsxmlb where   mtstuffitem.mtprod=mtjcxm.iid and gsxmlb.hisid=mtstuffitem.projecttype and mtstuffitem.ybsfsc=0  and mtmzblstuff in( " + iid4 + " )";
            ...
            string sql_up = "update mtstuffitem set ybsfsc=1 where mtmzblstuff in (" + id+")";
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            string sql_up = "update mtstuffitem set ybsfsc=0 where mtmzblstuff in("+iid4+")";
            BllMain.Db.Update(sql_up);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            string sql_mzbr = " select mtstuffitem.iid as idx,mtstuffitem.iid as cfh,mtstuffitem.fph as fph,Substring(mtstuffitem.createdat,0,11) as cfsj,'1' as cfts,mtprod.gszf,mtprod.gsbm as xmbm,mtprod.name as xmmc, gsxmlb.id as sflb,'' as sfdj,case  when (mtstuffitem.projecttype='2') or (mtstuffitem.projecttype='3' ) or (mtstuffitem.projecttype='4') then '0' else '1' end as zl,mtstuffitem.prc,mtstuffitem.qty as num,mtstuffitem.amt,mtprod.prodguige as guige,prodjixing.name as jixing ";
            sql_mzbr += " from mtstuffitem,mtprod,prodjixing,gsxmlb where mtstuffitem.mtprod=mtprod.iid and gsxmlb.hisid=mtstuffitem.projecttype and prodjixing.iid=mtprod.prodjixing  and mtmzblstuff in( " + iid4 + " )";
            sql_mzbr += " union ";
            sql_mzbr += " select mtstuffitem.iid as idx,mtstuffitem.iid as cfh,mtstuffitem.fph as fph,Substring(mtstuffitem.createdat,0,11) as cfsj,'1' as cfts,mtjcxm.gszf,mtjcxm.gsbm as xmbm,mtjcxm.name as xmmc, gsxmlb.id as sflb,'' as sfdj,case  when (mtstuffitem.projecttype='2') or (mtstuffitem.projecttype='3' ) or (mtstuffitem.projecttype='4') then '0' else '1' end as zl,mtstuffitem.prc,mtstuffitem.qty as num,mtstuffitem.amt,'' as guige,'' as jixing ";
            sql_mzbr += " from mtstuffitem,mtjcxm,gsxmlb where mtstuffitem.mtprod=mtjcxm.iid and gsxmlb.hisid=mtstuffitem.projecttype   and mtmzblstuff in( " + iid4 + " )";
            ...
            {
                string sql_cx = " update mtstuffitem set tcsl='" + dtfh.Rows[i]["zkc266"].ToString().Trim() + "',tcje='" + dtfh.Rows[i]["alc408"].ToString().Trim() + "',tcyy='" + dtfh.Rows[i]["aka073"].ToString().Trim() + "' where iid=" + dtfh.Rows[i]["idx"].ToString().Trim();
                BllMain.Db.Update(sql_cx);
            }
            string sql_tc = "select mtstuffitem.iid as idx,mtprod.name as xmmc,mtstuffitem.createdat as sfsj,mtstuffitem.prc ,mtstuffitem.amt ,mtstuffitem.qty,tcsl,tcje,tcyy from mtstuffitem,mtprod where mtstuffitem.mtprod=mtprod.iid and mtmzblstuff in (" + iid4+") ";
            sql_tc += " union select mtstuffitem.iid as idx,mtjcxm.name as xmmc,mtstuffitem.createdat as sfsj,mtstuffitem.prc ,mtstuffitem.amt ,mtstuffitem.qty,tcsl,tcje,tcyy from mtstuffitem,mtjcxm where mtstuffitem.mtprod=mtjcxm.iid and mtmzblstuff in (" + iid4+")";
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            string sql_sfxgs = "select sfxgs from mtmzblstuff  where iid in(" + iid4+")";
        ...
        private void button12_Click(object sender, EventArgs e)
        {
            ...
            string sql_ys = "select ctct.fullname from mtmzblstuff,ctct where ctct.iid=mtmzblstuff.doctorid and mtmzblstuff.iid=" + _iid;
        ...
        {
            string sql = "select spbh from mtmzblstuff where iid=" + _iid;
            DataTable dt = BllMain.Db.Select(sql).Tables[0];
            if (tbx_rdsh.Text.Trim() == "")
            {
                return;
            ...
            string sql_up = "update mtmzblstuff set spbh='' where iid=" + _iid;
        ...
        {
            string sql_mzbr = " select mtstuffitem.iid as cfh,mtstuffitem.fph as fph,Substring(mtstuffitem.createdat,0,11) as cfsj,'1' as cfts,mtprod.gsbm as xmbm,mtprod.name as xmmc, gsxmlb.name as sflb,'' as sfdj,case  when (mtstuffitem.projecttype='2') or (mtstuffitem.projecttype='3' ) or (mtstuffitem.projecttype='4') then '0' else '1' end as zl,mtstuffitem.prc,mtstuffitem.qty as num,mtstuffitem.amt,mtprod.prodguige as guige,prodjixing.name as jixing,mtprod.iid as prodid ";
            sql_mzbr += " from mtstuffitem,mtprod,prodjixing,gsxmlb where mtstuffitem.mtprod=mtprod.iid and gsxmlb.hisid=mtstuffitem.projecttype and prodjixing.iid=mtprod.prodjixing and mtmzblstuff=" + _iid;
            sql_mzbr += " union ";
            sql_mzbr += " select mtstuffitem.iid as cfh,mtstuffitem.fph as fph,Substring(mtstuffitem.createdat,0,11) as cfsj,'1' as cfts,mtjcxm.gsbm as xmbm,mtjcxm.name as xmmc, gsxmlb.name as sflb,'' as sfdj,case  when (mtstuffitem.projecttype='2') or (mtstuffitem.projecttype='3' ) or (mtstuffitem.projecttype='4') then '0' else '1' end as zl,mtstuffitem.prc,mtstuffitem.qty as num,mtstuffitem.amt,'' as guige,'' as jixing,mtjcxm.iid as prodid ";
            sql_mzbr += " from mtstuffitem,mtjcxm,gsxmlb where mtstuffitem.mtprod=mtjcxm.iid and gsxmlb.hisid=mtstuffitem.projecttype  and mtmzblstuff=" + _iid;
            ...
            { this.tbx_fph.Text = ""; }
            string sql_yz = " select mtstuffitem.iid as aae080,mtstuffitem.createdat as alk007,mtstuffitem.createdat as alk008,mtprod.name as alk009,'' as alk011,''as alk012,mtstuffitem.qty as alk013,'' as alk014,'' as alk015,ctct.fullname as alk016,'' as alk017,ctct.fullname as alk020,'2' as alk022,org.name as alk023,'' as alk024 from mtmzblstuff,mtstuffitem,mtprod,ctct,org where mtstuffitem.mtprod=mtprod.iid and mtmzblstuff.doctorid=ctct.iid and org.iid=mtmzblstuff.org and mtmzblstuff.iid=mtstuffitem.mtmzblstuff and mtmzblstuff.iid=" + _iid
                         + " union "
                         + " select mtstuffitem.iid as aae080,mtstuffitem.createdat as alk007,mtstuffitem.createdat as alk008,mtjcxm.name as alk009,'' as alk011,''as alk012,mtstuffitem.qty as alk013,'' as alk014,'' as alk015,ctct.fullname as alk016,'' as alk017,ctct.fullname as alk020,'2' as alk022,org.name as alk023,'' as alk024 from mtmzblstuff,mtstuffitem,mtjcxm,ctct,org where mtstuffitem.mtprod=mtjcxm.iid and mtmzblstuff.doctorid=ctct.iid and org.iid=mtmzblstuff.org and mtmzblstuff.iid=mtstuffitem.mtmzblstuff and mtmzblstuff.iid=" + _iid;
        ...
        private void cxfy()
        {
            ...
            int index = dataGridView_brxx.CurrentRow.Index;
            string register_id = dataGridView_brxx.Rows[index].Cells["id"].Value.ToString();
            string sql = " SELECT "
                   ...
                   + " clinic_costdet.createdate"
                   + " FROM"
                       + " clinic_costdet"
                   + " LEFT JOIN cost_itemtype ON clinic_costdet.itemtype_id = cost_itemtype.id"
                   + " LEFT JOIN bas_depart ON clinic_costdet.exedep_id = bas_depart.id"
                   + " WHERE"
                       + " clinic_costdet.regist_id =" + DataTool.addFieldBraces(register_id)
                   + " AND clinic_costdet.charged = 'OO'"
                   + " ORDER BY"
                       + " depart,"
                       + " clinic_costdet.id";
            DataTable cfdb = BllMain.Db.Select(sql).Tables[0];
            dataGridView3.DataSource = cfdb.DefaultView;
            tabControl1.SelectedTab = tabPage2;
            dateTimePicker1.Text = cfdb.Rows[0]["cfsj"].ToString().Trim();
            string sql_iid = "select gsjlh from mtmzblstuff where iid in(" + iid4 + ")";
