...
{
    public partial class frmZycjfysc : Form
    {
        JKDB jkdb = new JKDB();
        public frmZycjfysc()
        {
        ...
        private void frmZycjfysc_Load(object sender, EventArgs e)
        {
        ...
        private void SelectZyjlByQyid()
        {
            string sql2 = "select inhospital.id as iid, inhospital.ihspcode as zyh"
                      ...
                      + " where 1=1 and inhospital.nhflag = 1101 and inhospital.insurstat != 'SETT' order by inhospital.id ";
        ...
        {
            string sql_ypqbgx = "update ihsp_costdet set ypspbz=1 where ybsc=0 and yptsxx!='' and ypspbz not in (1,2) and ihsp_id in(select iid from mtzyjl where nhflag=1101)";
        ...
        {
            string sql_ypqbgx = "update ihsp_costdet set ypspbz=2 where ybsc=0 and yptsxx!='' and ypspbz not in (1,2) and ihsp_id in(select iid from mtzyjl where nhflag=1101)";
            if (BllMain.Db.Update(sql_ypqbgx) == -1)
            {
                MessageBox.Show("审批失败！");
            }
        ...
        private void drsj()
        {
            string sql = "select id, ihspcode from inhospital where nhflag=1101 and inhospital.insurstat != 'SETT' order by id";
            DataSet ds = BllMain.Db.Select(sql);
            Zycjfysc_button zycjfysc = new Zycjfysc_button();
            string mesg = "";
            int rs = ds.Tables[0].Rows.Count;
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                lblFeeUpLoad.Text = "正在城乡上传费用：" + (i + 1).ToString() + "/" + rs.ToString() + "人数";
                lblFeeUpLoad.Update();
                ...
                RetMsg ret = zycjfysc.ybscfymx(iid, grbh, zyh, label1);
                mesg += ret.Mesg + "\r\n";
                string msg = "";
                zycjfysc.xfdhjejyb_pl(grbh, ds.Tables[0].Rows[i]["id"].ToString().Trim(), zyh, out msg);
                mesg += msg + "\r\n";
            }
