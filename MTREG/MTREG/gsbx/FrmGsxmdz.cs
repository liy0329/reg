...
namespace zhongluyiyuan.gsbx
{
    public partial class FrmGsxmdz : Form
    {
        ...
        {            
            ...
            string sql = @"SELECT bas_item.id AS iid,gsbm AS bm,bas_item.`name`,bas_item.pincode AS jm,prodjixing.name AS jixingname
                           FROM bas_item left join (select * from sys_dict where father_id=40) prodjixing on bas_item.dosageform_id=prodjixing.sn";
            string tiaojian = " where (bas_item.`name` LIKE '%" + tbxxm.Text.Trim() + "%' or bas_item.pincode like '%" + tbxxm.Text.ToLower().Trim() + "%') and bas_item.isstop = 'N' ";

            if (checkBox1.Checked == true)
            {
                tiaojian += " and bas_item.itemtype_id IN (10, 11, 12)  ";
            }
            if (checkBox2.Checked == true)
            {
                tiaojian += " and bas_item.itemtype_id not IN (10, 11, 12)  ";
            }
        ...
        private void cx()
        {
            ...
            if (checkBox1.Checked == true)
            {
                tiaojian += " and bas_item.itemtype_id IN (12)  ";
            }
            if (checkBox2.Checked == true)
            {
                tiaojian += " and bas_item.itemtype_id not IN (10, 11, 12)  ";
            ...
            sql += tiaojian + "order by bas_item.name ";
            ryzddata = BllMain.Db.Select(sql).Tables[0];
            if (ryzddata.Rows.Count > 0)
            {
                dataGridView1.DataSource = ryzddata;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string sql_gx = "update bas_item set gsbm='" + this.tbxbm.Text.Trim() + "' where id=" + tbxiid.Text.Trim();
